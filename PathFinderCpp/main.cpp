...
int main(int argc, char* argv[])
{
	...
	("Pathfinder - please press 1-3 to change heuristic", // window's title
    ...
    // Create Bot
    //======================================================================================
    cBotBase* pBot = new cBotAStar(); //Use the AStar bot
	...
	{   
        ...
        //Please use the keys 1 for Manhattan, 2 for Diagonal and 3 for Euclidean
        
        //Key input to select the Manhattan heuristic                 
        if (keystate[SDL_SCANCODE_1]) // Scan key input 1
        {         
            gAStar.heuristic_choice = 1; // Set heuristic to 1 (Manhattan)
            gAStar.Build(*pBot); // Build the bot with the chosen heuristic     
        }       
        //Key input to select the Diagonal heuristic
        if (keystate[SDL_SCANCODE_2]) // Scan key input 2
        {
            gAStar.heuristic_choice = 2; // Set heuristic to 2 (Diagonal)
            gAStar.Build(*pBot); // Build the bot with the chosen heuristic   
        }      
        //Key input to select the Euclidean heuristic
        if (keystate[SDL_SCANCODE_3]) // Scan key input 3 
        {
            gAStar.heuristic_choice = 3; // Set heuristic to 3 (Euclidean)
            gAStar.Build(*pBot); // Build the bot with the chosen heuristic   
        }       
